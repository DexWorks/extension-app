 ...
 * This reducer requires `tabs` and `sessions` keys so it works with entire store
 */
'use strict';
...
function calculateStylesheetsForSession(tab) {
    ...
    iterate(tab.stylesheets, items => {
        var isMap = items instanceof Map || (typeof items === 'object' && !Array.isArray(items) && !(items instanceof Set));
