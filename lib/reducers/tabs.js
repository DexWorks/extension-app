...
import {has, get, copy, iterate, normalizeUrl, deepSet} from '../utils';
...
export default function(state={}, action) {
    switch (action.type) {
        ...
        case TAB.SET_STYLESHEET_DATA:
            return setStylesheetData(state, action.id, action.zone, action.items);
    }
    return state;
}
...
function setStylesheetData(state, id, zone, items) {
    ...
    state = copy(state);
    state.tabs = set(state.tabs, id, tab);
    return state;
}
