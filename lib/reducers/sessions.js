...
import {has, hasValue, get, copy, iterate, makeMap} from '../utils';
...
export default function(state=new Map(), action) {
    switch (action.type) {
        ...
        case SESSION.LOAD:
            return loadSessions(state, action);
    }
...
function removeUserStylesheet(state, action) {
    var session = get(state, action.id);
    if (session && hasValue(session.userStylesheets, action.stylesheet)) {
...
/**
 * Bulk load session data, mostly from external storages
 */
function loadSessions(state, action) {
    // In most cases, session data is a plain JSON objects, have to convert
    // some to Map and Set
    var sessions = new Map();
    iterate(action.sessions, (session, id) => {
        sessions.set(id, {
            ...session,
            userMapping: makeMap(session.userMapping),
            userStylesheets: new Set(session.userStylesheets)
        });
    });
    return sessions;
}
